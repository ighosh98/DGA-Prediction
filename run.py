...
def create_figs(nfolds=10, force=False):
    ...
    for name, model_result in sorted(results['model_results'].items()):
        if model_result is not None:
            ...
            print(("AUC: %s %.4f"%(name, auc))) #Check syntax
            metrics.append({
                'name': name,
                'fpr': fpr,
                'tpr': tpr,
    ...
    with pyplot.style.context('bmh'):
        for plot_name, metrics_filter in list(metrics_filters.items()):
            ...
            for metrics_record in sorted(metrics_to_plot, key=lambda rec: rec['auc'], reverse=True):
                ...
                plt.plot(
                    metrics_record['fpr'],
                    metrics_record['tpr'],
                    label='%s (AUC = %.4f)' % (metrics_record['name'].upper(), metrics_record['auc'], ), #check format
                    rasterized=True,
                    linestyle=linestyle,
                    color=color,
                )
